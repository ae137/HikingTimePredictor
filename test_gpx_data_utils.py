...
class TestPointConversions(unittest.TestCase):
    ...
    def test_roundtrip(self):
        for i in range(25):
            ...
            point = GPXPointFromArray(data)
            data_from_point = GPXPointToArray(point)
...
class TestSegmentConversions(unittest.TestCase):
    def setUp(self):
        ...
        self.short_segment = GPXTrackSegment([GPXPointFromArray(self.short_segment_as_array[0])])
        
        self.long_segment_as_array = np.random.normal(size=(25, 3))
        self.long_segment = GPXTrackSegment([GPXPointFromArray(point_data) 
                                             for point_data in self.long_segment_as_array])
        
    def test_segment_to_array_short(self):
        short_segment_as_array = GPXSegmentToArray(self.short_segment)
    ...
    def test_array_to_segment_short(self):
        short_segment = GPXSegmentFromArray(self.short_segment_as_array)
        
        self.assertEqual(len(short_segment.points), len(self.short_segment.points))
        
        for i in range(len(short_segment.points)):
            np.testing.assert_array_almost_equal(GPXPointToArray(short_segment.points[i]),
                                                 GPXPointToArray(self.short_segment.points[i]))
            
    def test_segment_to_array_long(self):
        long_segment_as_array = GPXSegmentToArray(self.long_segment)
    ...
    def test_array_to_segment_long(self):
        long_segment = GPXSegmentFromArray(self.long_segment_as_array)
        
        self.assertEqual(len(long_segment.points), len(self.long_segment.points))
        
        for i in range(len(long_segment.points)):
            np.testing.assert_array_almost_equal(GPXPointToArray(long_segment.points[i]),
                                                 GPXPointToArray(self.long_segment.points[i]))
            
    def test_bad_input_dim(self):
        with self.assertRaises(AssertionError):
            GPXSegmentFromArray(np.random.normal(size=(7, 4)))
            
        with self.assertRaises(AssertionError):
            GPXSegmentFromArray(np.random.normal(size=(7, 2)))
...
class TestSegmentDataFrameConversions(unittest.TestCase):
    ...
    def test_empty(self):
        result = GPXSegmentFromDataFrame(pd.DataFrame({'longitude': [], 'latitude': [], 'elevation': []}))
        self.assertEqual(len(result.points), 0)
        
    def test_simple(self):
        segment_from_df = GPXSegmentFromDataFrame(self.df)
        segment_from_array = GPXSegmentFromArray(self.df.values)
        
        self.assertEqual(len(segment_from_df.points), len(segment_from_array.points))
        
        for i in range(len(segment_from_df.points)):
            np.testing.assert_array_almost_equal(GPXPointToArray(segment_from_df.points[i]),
                                                 GPXPointToArray(segment_from_array.points[i]))
