...
class TestPointConversions(unittest.TestCase):
    def setUp(self):
        self.point_as_array = np.array([1.0, 2.0, 3.0])

        self.point = GPXTrackPoint(longitude=1.0, latitude=2.0, elevation=3.0)
    ...
    def test_array_to_point(self):
        ...
        for attribute in self.attributes_to_test:
            self.assertAlmostEqual(
                getattr(point, attribute), getattr(self.point, attribute)
            )
...
class TestSegmentConversions(unittest.TestCase):
    def setUp(self):
        self.short_segment_as_array = np.array([[1.0, 0.0, 3]])
    ...
    def test_segment_to_array_short(self):
        ...
        np.testing.assert_array_almost_equal(
            self.short_segment_as_array, short_segment_as_array
        )
    ...
    def test_array_to_segment_short(self):
        ...
        for i in range(len(short_segment.points)):
            np.testing.assert_array_almost_equal(
                gpx_point_to_array(short_segment.points[i]),
    ...
    def test_segment_to_array_long(self):
        ...
        np.testing.assert_array_almost_equal(
            self.long_segment_as_array, long_segment_as_array
        )
    ...
    def test_array_to_segment_long(self):
        ...
        for i in range(len(long_segment.points)):
            np.testing.assert_array_almost_equal(
                gpx_point_to_array(long_segment.points[i]),
...
class TestSegmentDataFrameConversions(unittest.TestCase):
    ...
    def test_empty(self):
        result = gpx_segment_from_data_frame(
            pd.DataFrame({"longitude": [], "latitude": [], "elevation": []})
        )
    ...
    def test_simple(self):
        ...
        for i in range(len(segment_from_df.points)):
            np.testing.assert_array_almost_equal(
                gpx_point_to_array(segment_from_df.points[i]),
